...
async function persistRoom(room) {
  ...
  if (room.players.length === 0) {
    ...
    if (age > ROOM_GRACE_MS) await store.deleteRoom(room.code);
...
async function joinRoom(code, opts = {}) {
  ...
  if (!room) return { error: 'Room not found — check the code or ask the host to create a new room' };
...
async function listRooms() {
  ...
  for (const code of codes) {
    ...
    const roomAge = now - (room.createdAt || room.updatedAt || 0);
    if (active.length === 0) {
      if (roomAge < ROOM_GRACE_MS) {
        list.push({
          code: room.code,
          game: room.game,
          players: 0,
