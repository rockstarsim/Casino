...
async function handleRooms(req, res) {
  ...
  res.setHeader('Access-Control-Expose-Headers', 'X-Store-Backend');

  if (req.method === 'GET') {
    const list = await engine.listRooms();
    res.setHeader('X-Store-Backend', store.usingRedis() ? 'redis' : 'memory');
...
async function handleAuth(req, res) {
  ...
  if (action === 'updateBalance') {
    const result = await auth.updateBalanceFromToken(token, body.balance);
    if (result.error) return res.status(401).json(result);
    return res.status(200).json(result);
  }
